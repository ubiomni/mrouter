...
#[cfg(feature = "sqlite")]
impl ProviderDao {
    pub fn get_by_id(db: &Database, id: i64) -> Result<Option<Provider>> {
    ...
    pub fn get_all_providers(db: &Database) -> Result<Vec<Provider>> {
        ...
        let providers = stmt.query_map([], |row| Self::row_to_provider(row))?;
    ...
    pub fn get_all(db: &Database, app_type: AppType) -> Result<Vec<Provider>> {
        ...
        let providers = stmt.query_map([app_type.as_str()], |row| Self::row_to_provider_with_app_type(row, app_type))?;
        let mut result = Vec::new();
        for provider in providers {
            result.push(provider?);
        }
    ...
    pub fn get_active(db: &Database, app_type: AppType) -> Result<Option<Provider>> {
        ...
        if let Some(row) = rows.next()? {
            Ok(Some(Self::row_to_provider_with_app_type(row, app_type)?))
        } else {
            Ok(None)
        }
    }
    ...
    pub fn name_exists(db: &Database, name: &str) -> Result<bool> {
        let conn = db.conn.lock().unwrap();
    ...
    pub fn name_exists_excluding(db: &Database, name: &str, exclude_id: i64) -> Result<bool> {
        let conn = db.conn.lock().unwrap();
        let count: i64 = conn.query_row(
            "SELECT COUNT(*) FROM providers WHERE name = ?1 AND id != ?2",
    ...
    pub fn set_is_active(db: &Database, id: i64, active: bool) -> Result<()> {
        db.execute(
    ...
    fn row_to_provider(row: &rusqlite::Row<'_>) -> rusqlite::Result<Provider> {
        ...
        Self::row_to_provider_with_app_type(row, app_type)
    }

    fn row_to_provider_with_app_type(row: &rusqlite::Row<'_>, app_type: AppType) -> rusqlite::Result<Provider> {
