...
pub fn render(f: &mut Frame, app: &App) {
    ...
    match app.current_tab {
        ...
        Tab::Daemon => render_daemon(f, chunks[2], app),
...
fn render_tabs(f: &mut Frame, area: Rect, app: &App) {
    let titles = vec!["[1] Providers", "[2] Daemon", "[3] Logs", "[4] Stats", "[5] Settings"];
    ...
    if let Some(provider) = app.providers.get(app.selected_provider) {
        ...
        let detail_lines = vec![
            ...
            Line::from(vec![
                Span::raw("  "),
                Span::styled(
                    "[v] Models  [p] Pricing  [m] Mappings  [o] Auth Header  [i] Input",
                    Style::default().fg(theme::TEXT),
                ),
            ]),
        ];
...
fn render_daemon(f: &mut Frame, area: Rect, app: &App) {
    // 三态状态：Starting (带动画) / Running / Stopped
    let (status_icon, status_color, status_text) = if app.proxy_starting {
        let ticks = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_millis();
        let dots = match (ticks / 400) % 4 {
            0 => "",
            1 => ".",
            2 => "..",
            _ => "...",
        };
        ("◌", theme::YELLOW, format!("Starting{}", dots))
    } else if app.proxy_running {
        ("●", theme::GREEN, "Running".to_string())
    } else {
        ("○", theme::RED, "Stopped".to_string())
    };
    ...
    let controls_text = if app.proxy_starting {
        "    Starting daemon, please wait..."
    } else if app.proxy_running {
        "    [p] Stop Daemon  [e] Edit Config  [r] Refresh Status"
    } else {
        "    [s] Start Daemon  [e] Edit Config  [r] Refresh Status"
    };
    ...
    let mut lines = vec![
        ...
        Line::from(""),
        Line::from(vec![
            Span::styled("  Proxy URL:      ", Style::default().fg(theme::YELLOW)),
    ...
    lines.extend([
        ...
        Line::from(""),
        Line::from(Span::styled(
            "  Note: Daemon runs in background. Exit TUI with [q] to keep it running.",
