...
pub enum Action {
    ...
    ConfigureInputModalities, // 配置输入模态（text, image 等）
    MultiSelectToggle,
    MultiSelectSubmit,
    ProxyStart,
    ProxyStop,
    ...
    if event::poll(Duration::from_millis(100))? {
        ...
        if let Event::Mouse(mouse) = ev {
            if let MouseEventKind::Down(_) = mouse.kind {
                ...
                if let Some(DialogKind::Input { fields, focused_field, title, .. }) = &app.dialog {
                    // 获取终端大小
                    ...
                    // 计算对话框的实际高度和可见字段（与 render_input_dialog 一致）
                    ...
                    let fields_height: u16 = if is_provider_dialog {
                        fields.iter().map(|f| {
                            if f.is_select() {
                                if f.label.contains("Type") { 6 } else { 4 }
                            } else { 3 }
                        }).sum()
                    } else {
                        fields.len() as u16 * 3
                    ...
                    if is_provider_dialog {
                        // Provider 对话框: field[0] (Type) 固定显示, 剩余字段可滚动
                        let type_height: usize = if fields[0].is_select() {
                            if fields[0].label.contains("Type") { 6 } else { 4 }
                        } else { 3 };

                        if relative_row < type_height {
                            // Clicked on field[0]
                            return Ok(Some(Action::MouseClickField(0)));
                        }

                        // 计算滚动偏移（与渲染逻辑一致）
                        let remaining_height = inner_height.saturating_sub(type_height).saturating_sub(2);
                        let field_height = 3usize;
                        let remaining_fields = fields.len().saturating_sub(1);
                        let mut max_vis = 0;
                        let mut used = 0usize;
                        for fi in 1..fields.len() {
                            let h = if fields[fi].is_select() {
                                if fields[fi].label.contains("Type") { 6 } else { 4 }
                            } else { field_height };
                            if used + h > remaining_height { break; }
                            used += h;
                            max_vis += 1;
                        }
                        let adjusted_focused = focused_field.saturating_sub(1);
                        let scroll_offset = if *focused_field == 0 {
                            0
                        ...
                        // 遍历可见字段计算点击位置
                        let mut y = type_height;
                        let end = (scroll_offset + max_vis).min(remaining_fields);
                        for i in scroll_offset..end {
                            let fi = 1 + i;
                            let h = if fields[fi].is_select() {
                                if fields[fi].label.contains("Type") { 6 } else { 4 }
                            } else { field_height };
                            if relative_row >= y && relative_row < y + h {
                                return Ok(Some(Action::MouseClickField(fi)));
                            }
                            y += h;
                        }
                    } else {
                        // 非 Provider 对话框: 固定 3 行每字段
                        let field_height = 3usize;
                        ...
                        let visible_field_index = relative_row / field_height;
...
fn handle_normal_mode(key: event::KeyEvent, app: &App) -> Result<Option<Action>> {
    match (key.code, key.modifiers) {
        ...
        (KeyCode::Char('2'), _) => Ok(Some(Action::SwitchTab(Tab::Daemon))),
        ...
        (KeyCode::Char('s'), _) => {
            ...
            } else if app.current_tab == Tab::Daemon {
        ...
        // Proxy 标签页特殊快捷键
        (KeyCode::Char('p'), _) if app.current_tab == Tab::Daemon => {
            Ok(Some(Action::ProxyStop))
        }
        (KeyCode::Char('e'), _) if app.current_tab == Tab::Daemon => {
        ...
        (KeyCode::Char('i'), _) if app.current_tab == Tab::Providers => {
            Ok(Some(Action::ConfigureInputModalities))
        }
