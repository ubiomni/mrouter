...
fn handle_non_streaming_passthrough(
    ...
    let stream = async_stream::stream! {
        ...
        let mut raw_log = if debug_enabled { Some(String::new()) } else { None };
        tokio::pin!(byte_stream);

        while let Some(chunk) = byte_stream.next().await {
            match chunk {
                Ok(bytes) => {
                    if let Some(ref mut log) = raw_log {
        ...
        if let Some(log) = raw_log {
            tracing::debug!("[Proxy] <<< Response body ({} bytes):\n{}", log.len(), log);
        }
    };
...
fn record_streaming_stats(
    ...
    if let Some(tid) = token_id {
        log_builder = log_builder.token_id(tid);
    }
    if let Some(tname) = token_name {
        log_builder = log_builder.token_name(tname);
    }
...
fn record_non_streaming_stats(
    ...
    if let Some(tid) = ctx.token_id {
        log_builder = log_builder.token_id(tid);
    }
    if let Some(ref tname) = ctx.token_name {
        log_builder = log_builder.token_name(tname.clone());
    }
