...
async fn run_app<B: ratatui::backend::Backend>(
    ...
    // 自动启动 daemon（如果尚未运行）
    if !app.proxy_running && !app.proxy_starting {
        let _ = app.handle_proxy_start().await;
    }

    loop {
        // 处理后台任务通知
        app.poll_background_notifications();
        ...
        if let Some(action) = tui::event::handle_events(app).await? {
            match action {
                ...
                tui::event::Action::ConfigureInputModalities => {
                    app.handle_configure_input_modalities();
                }
