...
use crate::services::ProviderSwitchService;
...
pub struct App {
    ...
    // Background task notification channel
    pub bg_notify_tx: mpsc::UnboundedSender<(String, NotificationLevel, Option<bool>)>,
    pub bg_notify_rx: mpsc::UnboundedReceiver<(String, NotificationLevel, Option<bool>)>,
}
...
impl App {
    ...
    pub async fn handle_input_submit(&mut self) -> Result<()> {
        ...
        if let Some(DialogKind::Input { title, fields, .. }) = &self.dialog {
            ...
            if title.contains("Configure Pricing") {
                ...
                self.input_mode = InputMode::Normal;
                return Ok(());
            } else if title.contains("Configure Input Modalities") {
                let default_raw = fields[0].value.trim().to_string();
                let model_raw = fields[1].value.trim().to_string();
                ...
                if let Some(provider) = self.providers.get(self.selected_provider) {
                    ...
                    let modalities: Vec<String> = default_raw
                        .split(',')
                        .map(|s| s.trim().to_lowercase())
                        .filter(|s| !s.is_empty())
                        .collect();

                    if modalities.is_empty() || modalities == vec!["text".to_string()] {
                        config_map.remove("input_modalities");
                    } else {
                        config_map.insert(
                    ...
                    // Model-level overrides: "model_id:mod1,mod2; model_id2:mod1,mod2"
                    let mut model_map = serde_json::Map::new();
                    if !model_raw.is_empty() {
                        for entry in model_raw.split(';') {
                            let entry = entry.trim();
                            if let Some((model_id, mods)) = entry.split_once(':') {
                                ...
                                let mods: Vec<String> = mods
                                    ...
                                    .filter(|s| !s.is_empty())
                                    .collect();
                                if !model_id.is_empty() && !mods.is_empty() {
                                    model_map.insert(
                                        model_id.to_string(),
                    ...
                    if model_map.is_empty() {
                        config_map.remove("model_input_modalities");
                    } else {
                        config_map.insert(
                            "model_input_modalities".to_string(),
                            serde_json::Value::Object(model_map),
                        );
                    }
                    ...
                    ProviderDao::update(&self.db, &updated)?;
                    self.show_notification(
                        format!("Updated input modalities for: {}", updated.name),
                        NotificationLevel::Success,
                    );
                    self.refresh().await?;
                }
            ...
            match self.current_tab {
                Tab::Providers => {
                    ...
                    if title.contains("Add") {
                        // 唯一性校验
                        if ProviderDao::name_exists(&self.db, &name).unwrap_or(false) {
                            self.show_notification(format!("Provider name '{}' already exists", name), NotificationLevel::Error);
                            return Ok(());
                        }
                        ...
                        if let Some(existing) = self.providers.get(self.selected_provider) {
                            // 唯一性校验（排除自身）
                            if ProviderDao::name_exists_excluding(&self.db, &name, existing.id).unwrap_or(false) {
                                self.show_notification(format!("Provider name '{}' already exists", name), NotificationLevel::Error);
                                return Ok(());
                            }
                ...
                Tab::Daemon => {
                    ...
                    if title.contains("Config") {
                        ...
                        let msg = if self.proxy_running {
                            "Config updated. Restart daemon to apply changes.".to_string()
                        } else {
    ...
    /// Drain background task notifications (non-blocking)
    pub fn poll_background_notifications(&mut self) {
        while let Ok((message, level, proxy_state)) = self.bg_notify_rx.try_recv() {
            if let Some(running) = proxy_state {
    ...
    fn save_settings_edit(&mut self, value: &str) -> Result<()> {
        ...
        match key {
            // === Logging ===
            "log.level" => {
                let simple = ["trace", "debug", "info", "warn", "error"];
                if simple.contains(&value) || tracing_subscriber::EnvFilter::try_new(value).is_ok() {
                    self.config.log.level = value.to_string();
                } else {
                    self.show_notification(
                        "Invalid log level. Use: trace/debug/info/warn/error or filter syntax (e.g. mrouter=debug,info)".to_string(),
                        NotificationLevel::Error,
                    );
                    return Ok(());
                }
        ...
        // 需要重启 daemon 才能生效的配置项
        let needs_restart = matches!(key,
            "proxy.port" | "proxy.bind" | "proxy.timeout" | "proxy.upstream_proxy"
            | "streaming.first_byte" | "streaming.idle" | "streaming.total"
            | "health.interval"
            | "cb.fail_threshold" | "cb.success_threshold" | "cb.timeout" | "cb.half_open_timeout"
        );

        if needs_restart && self.proxy_running {
            self.show_notification("Settings saved. Restart daemon to apply.".to_string(), NotificationLevel::Success);
        } else {
        ...
        if let Some(DialogKind::Input { fields, focused_field, .. }) = &mut self.dialog {
            if let Some(field) = fields.get_mut(*focused_field) {
                if field.is_select() {
                    ...
                    // Clamp cursor_pos to actual char count
                    if field.cursor_pos > char_count {
                        field.cursor_pos = char_count;
                    }
                    field.cursor_pos -= 1;
                    let byte_pos = field.cursor_byte_pos();
                    if byte_pos < field.value.len() {
                        field.value.remove(byte_pos);
                    } else if !field.value.is_empty() {
                        // Fallback: pop the last character
                        field.value.pop();
                    }
                }
            }
    ...
    /// 按 i 键配置输入模态（text, image 等）
    /// 支持 provider 级默认 + model 级覆盖
    pub fn handle_configure_input_modalities(&mut self) {
        if self.current_tab == Tab::Providers {
            if let Some(provider) = self.providers.get(self.selected_provider) {
                let modalities = provider.input_modalities();
                let modalities_str = modalities.join(", ");

                // model-level overrides: "model_id:mod1,mod2; model_id2:mod1,mod2"
                let model_map = provider.model_input_modalities_map();
                let model_str = model_map.iter()
                ...
                let mut fields = vec![
                    InputField::new(
                        "Default Modalities",
                        "Provider default (e.g., text, image)"
                    ),
                    InputField::new(
                        "Model Overrides",
                        "model:mod1,mod2; model2:mod1,mod2"
                    ),
                ];

                fields[0].set_value(modalities_str);
                fields[1].set_value(model_str);

                self.dialog = Some(DialogKind::Input {
                    title: format!("Configure Input Modalities: {}", provider.name),
                    fields,
                    focused_field: 0,
                });
                self.input_mode = InputMode::Editing;
    ...
    pub async fn handle_proxy_start(&mut self) -> Result<()> {
        ...
        if self.proxy_running {
            ...
            self.show_notification(
                format!("Port {} is already in use", self.proxy_port),
                NotificationLevel::Error,
            );
            return Ok(());
        }
        ...
        let ready_file = home.join(".mrouter").join("daemon.ready");
        ...
        match cmd.spawn() {
            Ok(_) => {
                ...
                // 后台等待 daemon ready 文件
                let tx = self.bg_notify_tx.clone();
                tokio::spawn(async move {
                    for _ in 0..150 {
                        tokio::time::sleep(tokio::time::Duration::from_millis(200)).await;
                        if ready_file.exists() {
                            let _ = tx.send(("".into(), NotificationLevel::Success, Some(true)));
                            return;
                        }
                    }
                    let _ = tx.send((
                        "Daemon start timeout. Check: mrouter daemon logs".into(),
                        NotificationLevel::Warning,
                        Some(false),
                    ));
                });
    ...
    pub async fn handle_proxy_edit_port(&mut self) -> Result<()> {
        let mut bind_field = InputField::new("Proxy Bind", "127.0.0.1");
        bind_field.set_value(self.proxy_bind.clone());

        let mut port_field = InputField::new("Proxy Port", "4444");
        port_field.set_value(self.proxy_port.to_string());

        self.dialog = Some(DialogKind::Input {
            title: "Edit Daemon Config".to_string(),
